import numpy as np #numerical-operations(arrays n polynomials)
import sys #Provides system-specific functions, such as modifying the module search path.
from sympy import Poly, symbols #polynomial operations
from NTRU.NTRUutil import * #utility functions(generate_polynomial,bitwise opeartions,padding arrays)
...
class NTRUencrypt:
    ...
    def __init__(self, N=503, p=3, q=256, d=18):
        ...
        self.genr() #genarates random_polynomial
        ...
        self.readKey = False # Flag to check if the public key is read

        # Variables to save any possible encrypted messages (if req)
        self.Me = None # Stores the final encrypted message as a string
    ...
    def setM(self,M):
        ...
        self.m = padArr(M,self.N) #Ensures uniform polynomial size for encryption
